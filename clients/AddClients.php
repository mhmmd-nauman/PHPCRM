...
$MerchantID = $objCompany->GetAllCompany(" ID = '".$CompanyID[0]['CompanyID']."'",array("MerchantID, SendInvoice"));

$sendInvoice = false;
if($MerchantID[0]['SendInvoice']==1){
	$sendInvoice = true;
}
...
if($_REQUEST['Task'] == 'RemovePackage'){
	die("Testing. Under Construction.");
...
if(isset($_REQUEST['PurchaseNow'])){
	...
	$merchantInfo = $objCompany->GetAllCompany("ID = " . $companyID, array("*"));
	$merchantType = $merchantInfo[0]['MerchantType'];
		if(!$merchantType){
			$merchantType =1;
		}
	...
	if(!empty($_REQUEST['BestCallTime'])){
		...
		$BestCallTime = date("Y-m-d ",strtotime($_REQUEST['BestCallTime'])) .$hour.":".$_REQUEST['minuts'].":00";
	}else{
		$BestCallTime = ""; 
	}
	//echo $BestCallTime;
	...
	if($merchantType==1){
		...
		//infusionsoft
		$merchantRecords['MerchantId'] = $haystack;
		$merchant_to_charge = $haystack[0];	
	}
	...
	if($merchantType==2){
			...
			// check $result - this is the status of the payment
			//$orderStatus = "Pending";
			...
			*/
			...
			// need logic to prevent this if the charge failed in IS
			if($success!=false || true){
				require_once('SaveOrderDB.php');
				if(($_REQUEST['send_email_invoice'] == "on" and !empty($_REQUEST['send_email_invoice'])) || true){
					// might need to turn back on for Kayako ticket generation
					require_once('SendInvoice.php');
					//require_once("../ecommerce/orders/OrdersInvoice.php?forcePaid=true");
				}
	...
	if(($_POST['voidtransaction'] == "on" || $_POST['voidtransaction'] == 1) and !empty($_POST['voidtransaction'])){
		...
		if($Order_created_without_charge == 1){
			...
			$ResponseMessage = "Order and client record created successfully! Unpaid order.";
			$pmt = 0;
			//echo "merchantType=" . $merchantType . " and unpaid";
			unset($_SESSION['OrderDataInRequest']);
			@header("location:AddClients.php?tab=1&flag=client_add&ResponseMessage=$ResponseMessage");  
		}
	}else{
		...
		//echo "merchantType=" . $merchantType . " and paid";
...
$products_array = $objproducts->GetAllProduct(" ShowOnOrderForm = 1 ORDER BY ProductName ",array("ID,ProductName,ProductPrice,Description, CompanyID"));
...
<script type="text/javascript">
	...
	function saveYearsinBiz(){
	var founded = $("#businessFounded").val();
	//alert(founded);
			$.ajax({
                type:"post",
                url:"<?php echo SITE_ADDRESS;?>ajax/AllAjaxCalls.php",
	            data:"id=1&Task=UpdateFounded&founded="+founded,
...
function saveServices(){
	var services = $("#businessServices").val();
	//alert(founded);
	$.ajax({
                type:"post",
                url:"<?php echo SITE_ADDRESS;?>ajax/AllAjaxCalls.php",
	            data:"id=1&Task=UpdateServices&services="+services,
				success:function(data){
	...
	function Validatetheform(){
		...
		if($("#selectproduct").val() == "" && price == 0){
			if($("#orderform select").length > -1){
				alert("you have a product with the price of zero");	
			} else {
		...
		var validcvv = validateforcvvdidgits();
		if(validcvv == false){
			return false;
		}
...
<div id="tabs">
                ...
                <tr>
                    ...
                    <select name="ProductID" class="dropdwn" id="selectproduct"> 
                        ...
                        foreach((array)$products_array as $products_row){
							...
							$companies = explode(",",$products_row['CompanyID']);
							$showRow = false;
							foreach($companies as $company){
								if($_SESSION['Member']['CompanyID']==trim($company)){
									$showRow = true;
								}
							} 
							
							if($showRow===true || $superDuper ===true){
						?>
 ...
 <form action="AddClients.php?Task=UpdateOrder" method="post" id="orderform"  target="_self" enctype="multipart/form-data" name="myForm">
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="2">
            ...
            <tr id="package-heading">
               ...
               foreach((array)$_SESSION['OrderData']['Products'] as $product_id){
                 if($product_id > 0 ){
           		 $package_products = $objproducts->GetAllProduct(" ID = '".$product_id."'",array("ID,ProductName,ProductPrice,Description,ShowCoupn, InternalPrice"));
				 
				 	$xosPrice = $package_products[0]['InternalPrice'];
                    ...
                    ?>
                     ...
                     <?php } ?>			 </td>
                     <td>
                     <?php //echo $current_product_price; ?>
                     	<div style="float:left;">
                        $
						<?php
                        $price_array = $objproducts->GetAllProductPrice(" ProductID = $product_id"  . " ORDER BY ProductPrice ASC",array("*"));
                        $singleprice = count($price_array);
                        if($singleprice > 1){
                        ?>
                     ...
                     <td align="center">
                         ...
                         <input type="hidden" name="product_array_price[<?php echo $product_id; ?>]" value="<?php echo $current_product_price; ?>" />
                          ...
                          <input type="hidden" name="product_XOSL9_array_price[<?php echo $product_id; ?>]" value="<?php echo $xosPrice; ?>" />
                         <?php $XOStotal += $xosPrice;?>
        ...
        <tr id="totalamount">
                 ...
                 <td>$<?php echo number_format($productTotal,2);?>
                 <input type="hidden" name="XOStotal" value="<?php echo $XOStotal; ?>" />
                 </td>
                 <td>&nbsp;</td>
             </tr>
             <?php }?>
   ...
   ?>   
        ...
        <tr >
            <td colspan="3" id="tabsubheading"> Business Information</td>
        </tr>
        <tr>
           <td width="30%"> Business Name: </td>
           <td width="65%">
               <input class="inputsapp required" name="cname" value="<?php echo $_SESSION['OrderDataInRequest']['cname'];?>" onblur="sendtoSession(this);" style="width:100%;" type="text"/>
           </td>
           <td width="30%">&nbsp;</td>
        </tr>
        ...
        <tr valign="top">
           <td > First Name:</td>
           <td >
               <input class="inputsapp required" name="fName" value="<?php echo $_SESSION['OrderDataInRequest']['fName'];?>" type="text" onblur="sendtoSession(this);" style="width:100%;" id="fName" />
           </td>
           <td >&nbsp;</td>
        </tr>
        <tr valign="top">
          <td > Last Name:</td>
          <td allign="left">
              <input  class="inputsapp required" name="sureName" value="<?php echo $_SESSION['OrderDataInRequest']['sureName'];?>" type="text" onblur="sendtoSession(this);" style="width:100%;" id="sureName" />
          </td>
          <td>&nbsp;</td>
        </tr>
        ...
        <tr valign="top">
          ...
          <td allign="left">
              <input class="inputsapp required email" name="Email" value="<?php echo $_SESSION['OrderDataInRequest']['Email'];?>" type="email" onblur="sendtoSession(this);" style="width:100%;"  id="Email"/>
          </td>
          <td>&nbsp;</td>
        </tr>
        ...
        <tr>
          <td >Best Phone: </td>
          <td allign="left">
              <input class="inputsapp required" name="phone" value="<?php echo $_SESSION['OrderDataInRequest']['phone'];?>" type="text" onblur="sendtoSession(this);" style="width:100%;" id="phone"/>
          </td>
          <td>&nbsp;</td>
        </tr>
        <tr>
          <td >Mobile Phone: </td>
          <td allign="left">
              <input class="inputsapp required" name="mobilephone" value="<?php echo $_SESSION['OrderDataInRequest']['mobilephone'];?>" type="text" onblur="sendtoSession(this);" style="width:100%;" />
          </td>
          <td>&nbsp;</td>
        </tr>
        <tr>
          ...
          <td allign="left">
              <input  class="inputsapp"name="alternatephone" value="<?php echo $_SESSION['OrderDataInRequest']['alternatephone'];?>" onblur="sendtoSession(this);" type="text" style="width:100%;" />
          </td>
          <td>&nbsp;</td>
        </tr>
        ...
        <tr>
            <td><label>Business Address 1:</label></td>
            <td> 
                <input class="inputsapp required" name="Streetaddress1" onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['Streetaddress1'];?>" type="text" style="width:100%;" />   
            </td>
            <td>&nbsp;</td>
        </tr>
        
        <tr>
            <td><label>Business Address 2:</label></td>
            <td> 
                <input class="inputsapp" name="Streetaddress2" onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['Streetaddress2'];?>" type="text" style="width:100%;" />   
            </td>
            <td>&nbsp;</td>
        </tr>
        <tr>
            ...
            <td> 
                <input class="inputsapp required" name="Bcity" onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['Bcity'];?>" type="text" style="width:100%;" />   
            </td>
            <td>&nbsp;</td>
        </tr>
        <tr>
            ...
            <td> 
                <select name="Bstate" class="product required" id="Bstate" onchange="sendtoSession(this);">
                ...
                <!--<input class="inputsapp required" name="Bstate" onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['Bstate'];?>" type="text" style="width:100%;" />-->            </td>
            <td>&nbsp;</td>
        </tr>
    <tr>
        <td>
        	...
        	<td> 
        		<input class="inputsapp required" onblur="sendtoSession(this);" name="Bpostalcode" value="<?php echo $_SESSION['OrderDataInRequest']['Bpostalcode'];?>" type="text" style="width:100%;" />   
        	</td>
        <td>&nbsp;</td>
    </tr>
    <tr>
        ...
        <tr>
    	<td colspan="3" id="tabsubheading">Fulfillment Information</td>
    </tr>
    <tr>
    <td><label>Business Web Site URL:</label></td>
        <td> 
            <input type="text" name="businessWebsite"  class="inputsapp"  onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['businessWebsite'];?>" />
        </td>
    	<td>&nbsp;</td>
    </tr>
    ...
    <td><label>Hours Of Operation:</label></td>
        <td> 
            <input type="text" name="businessHours"  class="inputsapp required"  onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['businessHours'];?>" />
        </td>
    	<td>&nbsp;</td>
    </tr>
    ...
    <td><label>Years in Business:</label></td>
        <td> 
            <input type="text" name="businessFounded" id="businessFounded"  class="inputsapp required"  onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['businessFounded'];?>" />
        </td>
    	<td>&nbsp;</td>
    </tr>
    <tr>
    <td><label>Services:</label></td>
        <td> 
            <textarea name="businessServices" id="businessServices" rows="4" style="height: auto;" class="inputsapp required"  onblur="sendtoSession(this);" ><?php echo $_SESSION['OrderDataInRequest']['businessServices'];?></textarea>
        </td>
    	<td>&nbsp;</td>
    </tr>
  
            <tr>
            <td><label>Fulfillment Notes:</label></td>
            <td> 
                <textarea class="inputsapp" name="OthersNotes" onblur="sendtoSession($(this));" rows="5" style="width:100%; height: auto"><?php echo $_SESSION['OrderDataInRequest']['OthersNotes'];?></textarea>   
            </td>
            <td>&nbsp;</td>
        </tr>
        
        <tr>
        	<td valign="top"><label>Add Welcome Call</label></td>
            <td><input type="checkbox" name="welcomeCall" id="welcomeCall" value="1" <?php if($_SESSION['OrderDataInRequest']['welcomeCall']=="1"){echo "checked"; }?> onclick="sendtoSession(this); " />
            <div class="hidden" id="welcomeCallBox" style="display: none">
            	<table class="inputsapp">
                <tr>
                	<td><label>Time</label><td>
                    <td><input type="text" name="BestCallTime" id="BestCallTime"  onblur="sendtoSession(this)" value="<?php echo $_SESSION['OrderDataInRequest']['BestCallTime'];?>" /></td>
                    <td><label>Time Zone</label></td>
                    <td><select class="inputsapp" name="TimeZone" style="width: auto!important"  onchange="sendtoSession(this)">
                     	<option value="PST" <?php if($_SESSION['OrderDataInRequest']['TimeZone']=="PST"){echo "selected";}?>>PST</option>
                        <option value="MST" <?php if($_SESSION['OrderDataInRequest']['TimeZone']=="MST"){echo "selected";}?>>MST</option>
                        <option value="CST" <?php if($_SESSION['OrderDataInRequest']['TimeZone']=="CST"){echo "selected";}?>>CST</option>
                        <option value="EST" <?php if($_SESSION['OrderDataInRequest']['TimeZone']=="EST" || empty($_SESSION['OrderDataInRequest']['TimeZone'])){echo "selected";}?>>EST</option>
                    </select>
                </tr>
                <tr>
                	<td colspan="4">
                    	<label>Hour</label>
                        <select class="inputsapp" name="hour" style="width: auto!important;"  onchange="sendtoSession(this)">
                        	<option value="01" <?php if($_SESSION['OrderDataInRequest']['hour']=="01"){echo "selected";}?>>01</option>
                            <option value="02" <?php if($_SESSION['OrderDataInRequest']['hour']=="02"){echo "selected";}?>>02</option>
                            <option value="03" <?php if($_SESSION['OrderDataInRequest']['hour']=="03"){echo "selected";}?>>03</option>
                            <option value="04" <?php if($_SESSION['OrderDataInRequest']['hour']=="04"){echo "selected";}?>>04</option>
                            <option value="05" <?php if($_SESSION['OrderDataInRequest']['hour']=="05"){echo "selected";}?>>05</option>
                            <option value="06" <?php if($_SESSION['OrderDataInRequest']['hour']=="06"){echo "selected";}?>>06</option>
                            <option value="07" <?php if($_SESSION['OrderDataInRequest']['hour']=="07"){echo "selected";}?>>07</option>
                            <option value="08" <?php if($_SESSION['OrderDataInRequest']['hour']=="08"){echo "selected";}?>>08</option>
                            <option value="09" <?php if($_SESSION['OrderDataInRequest']['hour']=="09"){echo "selected";}?>>09</option>
                            <option value="10" <?php if($_SESSION['OrderDataInRequest']['hour']=="10"|| empty($_SESSION['OrderDataInRequest']['hour'])){echo "selected";}?>>10</option>
                            <option value="11" <?php if($_SESSION['OrderDataInRequest']['hour']=="11"){echo "selected";}?>>11</option>
                            <option value="12" <?php if($_SESSION['OrderDataInRequest']['hour']=="12"){echo "selected";}?>>12</option>
                        </select> 
                        
                        <label>Minute</label>
                         <select class="inputsapp" name="minuts" style="width: auto!important;"  onchange="sendtoSession(this)">
                        	<option value="00" <?php if($_SESSION['OrderDataInRequest']['minuts']=="00"){echo "selected";}?>>00</option>
                            <option value="15" <?php if($_SESSION['OrderDataInRequest']['minuts']=="15"){echo "selected";}?>>15</option>
                            <option value="30" <?php if($_SESSION['OrderDataInRequest']['minuts']=="30"){echo "selected";}?>>30</option>
                            <option value="45" <?php if($_SESSION['OrderDataInRequest']['minuts']=="45"){echo "selected";}?>>45</option>
                        </select> 
                        <select class="inputsapp" name="AMPM" style="width: auto!important;"  onchange="sendtoSession(this)">
                        	<option value="AM" <?php if($_SESSION['OrderDataInRequest']['AMPM']=="AM"){echo "selected";}?>>AM</option>
                            <option value="PM" <?php if($_SESSION['OrderDataInRequest']['AMPM']=="PM"){echo "selected";}?>>PM</option>
                        </select>
                    </td>
                </tr>
                </table>
    ...
    <tr>
        ...
        <td>
        	<select name="select_pay_process" class="product required" id="select_pay_process" onchange="sendtoSession(this);">
    ...
    <tr class="process_later">
        ...
        <td width="65%">
            <input name="process_later_date" onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['process_later_date'];?>" type="text" class="inputsapp required" id="process_later_date" /></td>
        <td width="30%">&nbsp;</td>
    </tr>
<tr class="creditcarddetails">
    <td>Credit Card Type:</td>
        <td colspan="2">
            <select name="creditcardtype" id="creditcardtype" class="product required" onchange="sendtoSession(this);">
            ...
            <label style="display:none;" for="Ccode" class="error creditcarderror"></label>
        </td>
    </tr>
    <tr class="creditcarddetails">
        <td width="30%">Credit Card Number:</td>
        <td width="65%">           
        	<input type="text"class="inputsapp required" onblur="sendtoSession(this);" value="<?php echo $_SESSION['OrderDataInRequest']['Cnumber']; ?>" name="Cnumber" id="Cnumber" >
        </td>
    ...
    <tr class="creditcarddetails">
        ...
        <td>
        <select name="ExpiryMonth" id="ExpiryMonth" class="product required" style="margin-bottom:5px;" onchange="sendtoSession(this);">
        ...
        <select name="ExpiryYears" id="ExpiryYear" class="product required" onchange="sendtoSession(this);">
    ...
    <tr class="creditcarddetails">
        <td>Security Code:</td>
        <td colspan="2"> 
            <input value="<?php echo $_SESSION['OrderDataInRequest']['Ccode'];?>" name="Ccode" type="text" class="product required" id="Ccode" onblur="sendtoSession(this);"/> 
            <label style="display:none;" for="Ccode" class="error cvverror"></label>
        </td>
        
    </tr>
    <tr class="creditcarddetails">
    	<td valign="top">Billing Address</td>
        <td><label><input type="checkbox" name="sameAddress" id="sameAddress" value="1" onClick="addressToggle(this)">Same as business address</label>
        <br />
        
        <textarea id="accountAddress" class="required" name="accountAddress" style="width:59.8%;" onblur="sendtoSession(this);"><?php echo$_SESSION['OrderDataInRequest']['accountAddress']; ?></textarea>
    ...
    <tr class="echeck">
        <td>Bank Name:</td>
        <td><input value="<?php echo $_SESSION['OrderDataInRequest']['bank_name']; ?>" name="bank_name" type="text" class="product required" onblur="sendtoSession(this);"/></td>
        <td>&nbsp;</td>
    </tr>
    <tr class="echeck">
        <td>Name On The Account:</td>
        <td><input value="<?php echo $_SESSION['OrderDataInRequest']['name_on_account']; ?>" name="name_on_account" type="text" class="product required" onblur="sendtoSession(this);"/></td>
        <td>&nbsp;</td>
    </tr>
    ...
    <tr class="echeck">
        ...
        <td>
            <select class="product required" name="account_holder_type" id="account_holder_type" onchange="sendtoSession(this);">
    ...
    <tr class="echeck">
        <td>Account Type:</td>
        <td>
            <select name="account_type" id="account_type" class="product required" onchange="sendtoSession(this);">
    ...
    <tr class="echeck">
        <td>Check Type:</td>
        <td>
            <select name="check_type" id="check_type" class="product required" onchange="sendtoSession(this);">
    ...
    <tr class="echeck">
        <td>Check Number:</td>
        <td><input value="<?php echo $_SESSION['OrderDataInRequest']['check_number']; ?>" name="check_number" onblur="sendtoSession(this);" type="text" class="product required"/></td>
        <td>&nbsp;</td>
    </tr>
    <tr class="echeck">
        <td>Routing Number:</td>
        <td><input value="<?php echo $_SESSION['OrderDataInRequest']['routing_number']; ?>" name="routing_number" onblur="sendtoSession(this);" maxlength="9" type="text" class="product required"/></td>
        <td>&nbsp;</td>
    </tr>
    <tr class="echeck">
        <td>Account Number:</td>
        <td><input value="<?php echo $_SESSION['OrderDataInRequest']['account_number']; ?>" name="account_number" onblur="sendtoSession(this);" type="text" class="product required"/></td>
        <td>&nbsp;</td>
    </tr>

	<tr class="echeck">
    	<td valign="top">Address on Account</td>
        <td><label><input type="checkbox" name="sameAddress" id="sameAddress" value="1" onClick="addressToggle(this)">
        Same as business address</label>
        <br />
        
        <textarea id="accountAddress" name="accountAddress" style="width:59.8%;" onblur="sendtoSession(this);"><?php echo$_SESSION['OrderDataInRequest']['accountAddress']; ?></textarea>
    ...
    <tr>
    	<td>Billing Notes:</td>
        <td><textarea name="paymentnoteswhileorder" rows="5" style="width:59.8%;" onblur="sendtoSession(this);"><?php echo $_SESSION['OrderDataInRequest']['paymentnoteswhileorder']; ?></textarea></td>
        <td>&nbsp;<input type="hidden" name="voidtransaction" value="" id="voidtransaction"></td>
    </tr>
    <tr id="invoiceNow" class="creditcarddetails" style="visibility:hidden">
...
function addressToggle(node) {
	if(node.checked){
	//alert($("input[name='Streetaddress1']").val());
	
	var addy = $("input[name=Streetaddress1]").val();
	addy += " " + $("input[name=Streetaddress2]").val();
	addy += "\n" + $("input[name=Bcity]").val();
	addy += ", " + $("select[name=Bstate]").val();
	addy += " " + $("input[name=Bpostalcode]").val();
	//alert(addy);
		$("#accountAddress").val(addy);
	} else {
		$("#accountAddress").val("");	
	}
	$('#accountAddress').focus().blur();
}
...
$(document).ready(function(){
	...
	$("#BestCallTime").datepicker();
	$("#welcomeCall").click(function(){
		$("#welcomeCallBox").toggle();	
	});
	if($("#welcomeCall").is(":checked")){
		$("#welcomeCallBox").show();	
	}
	...
	$("#select_pay_process").change(function(){
		...
		if(selected == 1 || selected == 2){
			if(selected == 2){
				...
				$(".creditcarddetails").show("fast");
				$("#invoiceNow").hide("fast");
			}else{
				$(".creditcarddetails").show("fast");
				$("#invoiceNow").css("display", "table-row");
				$("#voidtransaction").val(0);
				$(".process_later").hide("fast");
			}
		}else{
			$("#invoiceNow").css("display",  "none");
			$(".creditcarddetails").hide("fast");
			$(".process_later").hide("fast");
		}
		if(selected == 3){
...
function sendtoSession(this_pointer){
	if($(this_pointer).val() != ""){
		$(this_pointer).css({"border":"1px solid #DBDBDB"});
	}
	
	if($(this_pointer).attr("id") == "Ccode" || $(this_pointer).attr("id") == "creditcardtype"){
		$(".creditcarderror").remove();
		validateforcvvdidgits();
	}
	
	
	if(this_pointer.name=="OthersNotes"){
		this_pointer.value = encodeURIComponent(this_pointer.value);	
	}
...
function validateforcvvdidgits(){
	var cvvcode = $.trim($("#Ccode").val());
	var payprocess = $("#creditcardtype").val();
	/* This will validate the cvv field in respect to credit card type.
	   Meaning, we need to validate the cvv field in three conditions
	   1) If the agent have entered the cvv before selecting the card type.
	   2) If the agent enters the cvv after selecting the card type.
	   3) If the agent changes the cvv code after selecting the card type.
	   4) If the agent entered wrong number of digits and even though sees
	      the error message but tries to hit the Order button. 
	 */
	if(isNaN(cvvcode)){
		$(".cvverror").html("Enter valid number.").show();
		$("#Ccode").css({"border":"1px dashed #F08080"});
		return false;
	}
	if(cvvcode != ""){
		var count = cvvcode.match(/\d/g).length;
		if(payprocess == "American Express"){
			if(count != 4){
				$(".cvverror").html("CVV code requires a 4 digit number.").show();
				$("#Ccode").css({"border":"1px dashed #F08080"});
				return false;
			}else{
				$(".cvverror").html("").hide();
			}
		}else if(payprocess == "Master Card" || payprocess == "Visa" || payprocess == "Discover"){
			if(count != 3){
				$(".cvverror").html("CVV code requires a 3 digit number.").show();
				$("#Ccode").css({"border":"1px dashed #F08080"});
				return false;
			}else{
				$(".cvverror").html("").hide();
			...
			$("#Ccode").css({"border":"1px solid #DBDBDB"});
			$(".cvverror").html("").hide();
		}
	}
	
	if($("#creditcardtype").val() != ""){
		var payprocess = $.trim($("#creditcardtype").val());
		if(cvvcode != ""){
			var count = cvvcode.match(/\d/g).length;
			if(payprocess == "American Express"){
				if(count != 4){
