...
<script type="text/javascript">
 $(function() {
                    ...
                    <?php  foreach((array)$Users_array as $Users_row){?>        
                       $("#UserEdit<?php echo $Users_row['ID'];?>").click(function(e){
                                     e.preventDefault();	
                                     modalbox(this.href,this.title,550,800);
                         });	 
                <?php } ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <td width="136" > Created</td>
   <td width="200" >Name </td>
    ...
    <td ><span class="Action">Commision</span></td>
  </tr>
  <?php  foreach((array)$Users_array as $Users_row){
			...
			$client_submitted_array =  $objClient->GetAllClients("Clients.SubmitedBy = '".$Users_row['UserID']."'",array("count(*) as total"));
				...
				$OrderID= $objOrder->GetAllOrder("UserID=".$Users_row['UserID']."",array("ID"));
				$Order_Detail= $objOrder->GetAllOrderDetailWithProduct("OrderID=".$OrderID[0]['ID']."",array("*"));
				$ProductPerc=$objProduct->GetAllProduct("ID=".$Order_Detail[0]['ProductID']."",array("ProductPrecentage"));
				//print_r($ProductPerc[0]['ProductPrecentage']);
				//print_r($Order_Detail[0]['ProductName']);
				//print_r($Order_Detail[0]['ProductPrice']);
			   $comminsion = ($Order_Detail[0]['ProductPrice']*$Order_Detail[0]['Quantity'])/$ProductPerc[0]['ProductPrecentage'];
			  // print_r($OrderID1);
  ...
  <tr id="<?php echo $row_class;?>">
   ...
   echo date("<b>M d</b>, Y",strtotime( $Users_row['Created']));?></td>
          <td>
              <?php echo $Users_row['FirstName']." ".$Users_row['Surname'];?>          </td>
	<td width="220"><?php echo $Users_row['Email'];?>            </td>
    <td><?php echo $Order_Detail[0]['ProductName']; ?></td>
    <td><?php echo $Order_Detail[0]['ProductPrice']; ?></td>
    <td><?php echo $Order_Detail[0]['Quantity'];?></td>
    <td><?php echo $comminsion; ?></td>
  </tr>
 <?php }?>
</table>
<div align="center">
