<?php
  ...
  */
  ...
  class payflow {
    ...
    var $test_mode; // 1 = true, 0 = production
    
    function payflow($vendor, $user, $partner, $password,$test_mode) {
    ...
    function get_version() {
      return '4.03';
		    ...
		    // Add recurring
    function Add_Recurring($card_number, $card_expire, $amount, $currency = 'USD', $data_array = array()) {
	 ...
	 // exit;
      ...
      if (isset($pfpro['RESULT']) && $pfpro['RESULT'] == 0) {
        ...
        return false;     
		    ...
		    // Add recurring
    function Modify_Recurring($card_number='',$amount,$data_array = array()) {
	
		

      if(empty($data_array['ORIGPROFILEID'])){
			if ($this->validate_card_number($card_number) == false) {
			$this->set_errors('Card Number not valid');
			return;     
		  }
	 ...
	 // exit;
      ...
      if (isset($pfpro['RESULT']) && $pfpro['RESULT'] == 0) {
        ...
        return false;     
		    ...
		    // Cancel recurring
    function Cancel_Recurring($data_array = array()) {
	...
	// exit;
	 ...
	 // View  Profile Status
    function View_Recurring($data_array = array()) {
	...
	// exit;
	 ...
	 function View_Order($data_array = array()) {
	...
	// exit;
      ...
      $plist .= 'PWD=' . $this->password . '&';           
      $plist .= 'TENDER=' . 'C' . '&'; // C = credit card, P = PayPal
      $plist .= 'TRXTYPE=' . 'I' . '&'; //  S = Sale transaction, A = Authorisation, C = Credit, D = Delayed Capture, V = Void                        
      $plist .= 'ORIGID='.$data_array['ORIGID'].'&';
	  $plist .= 'ACTION=' . $data_array['ACTION'] . '&';
	  $plist .= 'CLIENTIP=' . $data_array['clientip'] . '&';
