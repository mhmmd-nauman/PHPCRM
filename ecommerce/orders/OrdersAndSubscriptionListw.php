...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="Order_List" style="display:none;">
  ...
  <tr id="<?php echo $row_class;?>" class="remove_<?php echo $OrdersRows['ID']; ?>">
   ...
   <td style="width:100px; text-align:center;" onmouseout='hidetooltip(this);' class="<?php if($OrdersRows['Status'] == 'Paid' || $OrdersRows['Status'] == "Active" || $OrdersRows['Status'] == 1) { ?>paid<?php }elseif($OrdersRows['Status'] == 'Unpaid'){ ?>unpaid<?php } elseif($OrdersRows['Status'] == 'Refunded' || $OrdersRows['Status'] == 'Cancelled'){ ?>refunded<?php }else{ ?>unpaid<?php } ?>" >
   ...
   <?php if($OrdersRows['Status'] == "Unpaid"){ ?>
   		...
   		<a title="Payment For Order ID - <?php echo $OrdersRows['ID']; ?>" onmouseover='showtooltip(this);' class="CollectPayment" href="../../clients/CollectPayment.php?OrderID=<?php echo $OrdersRows['ID'];?>&MembeID=<?php echo $OrdersRows['MemberID']; ?>">
   <?php
   }
